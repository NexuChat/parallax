...
from .base import FONT_FACE_CSS, Account, Request, Response
...
class ControlSite:
    ...
    def _page(self, request: Request, body: str, label: str) -> Response:
        ...
        return Response.html(f'<!doctype html><html lang="{lang}" dir="{"rtl" if lang == "ar" else "ltr"}" data-theme="{theme}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark">{self._css(request.theme)}<title>{escape(words["brand"])} — {escape(words[label])}</title></head><body><div class="shell"><header><div class="mast"><span class="brand">{escape(words["brand"])}</span>{account}</div><nav aria-label="{escape(words["home"])}">{nav}</nav></header><main>{body}</main><footer>{escape(words["footer"])}</footer></div></body></html>')
    ...
    blurb = "The clean control. Nothing is planted here, so any finding against it is a false positive."
    entry = "/"
