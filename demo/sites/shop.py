...
class ShopSite:
    ...
    def handle(self, request: Request) -> Response:
        ...
        if path == "/":
            ...
            content = self._checkout(copy, request.mount)
        ...
        return Response.html(self._page(copy, lang, theme, content, request.mount))

    def _page(self, copy: dict[str, str], lang: str, theme: str, content: str, mount: str) -> str:
        direction = "rtl" if lang == "ar" else "ltr"
        nav = "".join(f'<a href="{mount}{href}">{copy[label]}</a>' for href, label in (("/", "catalogue"), ("/cart", "cart"), ("/checkout", "checkout")))
...
</style></head><body><header><div class="shell bar"><a class="brand" href="{mount}/">{copy["brand"]}</a><nav aria-label="Primary">{nav}</nav></div></header><main class="shell">{content}</main></body></html>'''

    def _catalogue(self, copy: dict[str, str], lang: str, mount: str) -> str:
        cards = "".join(f'<article class="card"><div class="swatch" aria-hidden="true"></div><h2>{escape(values[lang][0])}</h2><p>{escape(values[lang][1])}</p><p class="price">{values[lang][2]}</p><a class="text-link" href="{mount}/product/{key}">{copy["view"]} →</a></article>' for key, values in _PRODUCTS.items())
        return f'<p class="eyebrow">{copy["eyebrow"]}</p><h1>{copy["headline"]}</h1><p class="intro">{copy["intro"]}</p><section class="grid">{cards}</section>'

    def _product(self, copy: dict[str, str], lang: str, product_id: str, mount: str) -> str:
        title, description, price = _PRODUCTS[product_id][lang]
        return f'<a class="text-link" href="{mount}/">← {copy["back"]}</a><section class="product-layout" style="margin-block-start:26px"><div class="product-art" aria-hidden="true"></div><div><p class="eyebrow">{copy["details"]}</p><div class="product-title-box"><h1>{escape(title)}</h1></div><p class="intro">{escape(description)}</p><p class="price">{price}</p><a class="button" href="{mount}/cart">{copy["add"]}</a></div></section>'

    def _cart(self, copy: dict[str, str], lang: str, mount: str) -> str:
        ledger, lamp = _PRODUCTS["ledger"][lang][0], _PRODUCTS["lamp"][lang][0]
        return f'''<h1>{copy["your_cart"]}</h1><table class="cart-table"><thead><tr><th>{copy["item"]}</th><th>{copy["quantity"]}</th><th>{copy["price"]}</th><th>{copy["subtotal"]}</th></tr></thead><tbody><tr><td>{ledger}</td><td><span class="stepper"><button type="button" aria-label="Decrease quantity">−</button><span>1</span><button type="button" aria-label="Increase quantity">+</button></span></td><td>$18</td><td>$18</td></tr><tr><td>{lamp}</td><td><span class="stepper"><button type="button" aria-label="Decrease quantity">−</button><span>1</span><button type="button" aria-label="Increase quantity">+</button></span></td><td>$64</td><td>$64</td></tr></tbody></table><p><a class="text-link" href="{mount}/">← {copy["continue"]}</a></p><a class="button" href="{mount}/checkout">{copy["checkout"]}</a>'''

    def _checkout(self, copy: dict[str, str], mount: str) -> str:
        return f'<section class="checkout-layout"><div><p class="eyebrow">{copy["secure"]}</p><h1>{copy["checkout"]}</h1><p class="intro">{copy["checkout_note"]}</p><div class="checkout-action-row"><a class="text-link" href="{mount}/cart">← {copy["cart"]}</a><button class="button" type="button">{copy["place"]}</button></div></div><aside class="summary"><h2>{copy["order"]}</h2><p><span>{copy["items"]}</span><span>$82</span></p><p><span>{copy["shipping"]}</span><span>$8</span></p><p><span>{copy["total"]}</span><span>$90</span></p></aside></section>'
