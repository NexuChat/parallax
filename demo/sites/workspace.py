...
class WorkspaceSite:
    ...
    def _page(self, request: Request, content: str, lang: str, theme: str, active: str) -> Response:
        ...
        return Response.html(f'<!doctype html><html lang="{lang}" dir="{direction}" data-theme="{chosen_theme}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><title>{escape(c["brand"])}</title><style>{self._css(theme)}</style></head><body>{header}{content}<footer class="site-footer">{escape(c["footer"])}</footer></body></html>', **({"Set-Cookie": cookie} if cookie else {}))
