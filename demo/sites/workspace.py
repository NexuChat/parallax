...
class WorkspaceSite:
    ...
    def _page(self, request: Request, content: str, lang: str, theme: str, active: str) -> Response:
        if lang == "ar":
            translations = {
                "I can take the handoff copy before lunch.": "يمكنني تولي نص التسليم قبل الغداء.",
                "A private note kept off the shared stream.": "ملاحظة خاصة بعيدة عن السجل المشترك.",
                "Staging is green after the accessibility pass.": "بيئة التجهيز جاهزة بعد مراجعة إمكانية الوصول.",
                "Three recurring requests are now tagged for review.": "وُسِمت ثلاثة طلبات متكررة للمراجعة.",
                "Morning — the launch checklist is ready for a final pass.": "صباح الخير — قائمة الإطلاق جاهزة للمراجعة الأخيرة.",
                "I added the customer notes from yesterday’s interviews.": "أضفت ملاحظات العملاء من مقابلات الأمس.",
                "Changed billing owner": "غيّر مالك الفوترة",
                "Invited member": "دعا عضوًا",
                "Updated notification preference": "حدّث تفضيل الإشعارات",
                "Exported workspace data": "صدّر بيانات مساحة العمل",
                "Team plan": "خطة الفريق",
                "Weekly digest": "الملخص الأسبوعي",
                "Avery Kim": "أفيري كيم",
                "Samira Khan": "سميرة خان",
                "Maya Chen": "مايا تشين",
                "Mira Patel": "ميرا باتل",
            }
            for source, translated in translations.items():
                content = content.replace(source, translated)
        c, direction = _COPY[lang], "rtl" if lang == "ar" else "ltr"
        links = (("threads", c["threads"]), ("settings", c["settings"]), ("billing", c["billing"]))
        if (user := self._user(request)) is not None and user["role"] == "owner":
            links += (("audit", c["audit"]),)
