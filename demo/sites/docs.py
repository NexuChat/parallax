...
class DocsSite:
    ...
    def _page(self, t: dict[str, str], lang: str, theme: str, path: str, mount: str) -> str:
        ...
        nav = "".join(f'<a href="{mount}{href}">{t[key]}</a>' for href, key in (("/", "home"), ("/guide", "guide"), ("/api", "api"), ("/faq", "faq")))
        pages = {"/": f'<p class="kicker">{t["eyebrow"]}</p><h1>{t["headline"]}</h1><p class="lead">{t["lead"]}</p><p class="help-text">{t["help"]}</p><a class="cta" href="{mount}/guide">{t["read"]} →</a>', "/guide": self._guide(t, lang), "/api": f'<p class="kicker">{t["eyebrow"]}</p><h1>{t["api_title"]}</h1><p class="lead">{t["api_text"]}</p><pre><code>GET /v1/projects</code></pre>', "/faq": self._faq(t)}
...
</style></head><body><header><div class="wrap top"><a class="brand" href="{mount}/">{t["brand"]}</a><nav aria-label="Primary">{nav}</nav></div></header><main class="wrap">{pages[path]}</main></body></html>'''
