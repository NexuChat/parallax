...
The hosted service is sized for demonstration, and the sizing is a written decision rather than an accident: `deploy/cloudrun.sh` pins the service to one instance and says why beside each flag. The run registry is a locked in-process dict, and run artifacts live on the instance's filesystem, which on Cloud Run is memory — a restart forgets unfinished runs, and a second instance would answer another instance's `GET /runs/<id>` with 404. That is the cost of keeping a sweep a background thread beside its own evidence, accepted so that a public demo stays one container with one bill. The upgrade path is mechanical rather than architectural: move the registry rows to Firestore and the artifact directories to Cloud Storage and `--max-instances` stops being load-bearing, because nothing in the sweep itself knows how many instances exist. The published sweeps under `web/` are baked into the image at build time, which is why the curated evidence survives the restarts that the live registry does not.
