...
# 512Mi default. The live registry stays in one instance's memory, so a second
...
# the CPU must stay allocated between requests. Finished runs are mirrored to
# the bucket below through the runtime service account, which is why a saved
# run link survives the restarts that the registry does not; create it once:
#   gcloud storage buckets create "gs://${PROJECT_ID}-parallax-runs" \
...
gcloud run deploy "$SERVICE" \
  ...
  --max-instances=1 \
  --no-cpu-throttling \
  --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=global,PARALLAX_RUNS_BUCKET=${PROJECT_ID}-parallax-runs" \
