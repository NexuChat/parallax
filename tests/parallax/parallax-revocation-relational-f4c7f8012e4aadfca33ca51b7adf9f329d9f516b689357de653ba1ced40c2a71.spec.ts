 ...
 * Finding: revocation-relational-5e2791251d7f5c2d
 * Axis: relational
...
test("Parallax: revocation-relational-5e2791251d7f5c2d", async ({ browser }) => {
  ...
  expect(await receiverPage.locator("button.secondary").isVisible().catch(() => false)).toBeTruthy();
  ...
  await expect.poll(async () => await receiverPage.locator("button.secondary").isVisible().catch(() => false), { timeout: 5000 }).toBeFalsy();
  const revocationLagMs = performance.now() - revocationCompletedAt;
  expect(revocationLagMs).toBeLessThanOrEqual(1000);
  } finally {
