...
Content signatures are a cheap screen, not the final content judgement. The FNV-1a signature identifies a changed region, while a matching signature never reaches a semantic model. For a changed landmark region, `gemini-embedding-001` on Vertex AI supplies embeddings whose cosine similarity is compared with the named `0.90` equivalence threshold; the resulting score travels with the finding as evidence. Theme and viewport use this comparison to distinguish ordinary copy variation from material content divergence.
