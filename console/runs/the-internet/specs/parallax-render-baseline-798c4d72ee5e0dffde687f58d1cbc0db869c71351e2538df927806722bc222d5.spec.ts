 ...
 * Finding: render-baseline-0d4acfd22185fa72-low_contrast
 * Axis: baseline
...
test("Parallax: render-baseline-0d4acfd22185fa72-low_contrast", async ({ page }) => {
  const response = await page.goto("/");
  const isLoginPage = /\/(?:login|sign-in|auth)(?:[/?#]|$)/i.test(new URL(page.url()).pathname);
  const contrastRatio = await page.locator("div.row:nth-of-type(2) > div.large-12.columns > ul > li:nth-of-type(1) > a").evaluate((element) => {
