 ...
 * Finding: render-baseline-1c73ba87c9cc618e-low_contrast
 * Axis: baseline
...
test("Parallax: render-baseline-1c73ba87c9cc618e-low_contrast", async ({ page }) => {
  const response = await page.goto("/challenging_dom");
  const isLoginPage = /\/(?:login|sign-in|auth)(?:[/?#]|$)/i.test(new URL(page.url()).pathname);
  const contrastRatio = await page.locator("#2bcf4480-86fb-013f-5669-6abe127e9cb4").evaluate((element) => {
