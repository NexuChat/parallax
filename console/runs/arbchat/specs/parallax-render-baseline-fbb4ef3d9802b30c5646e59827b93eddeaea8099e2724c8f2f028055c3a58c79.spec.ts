 ...
 * Finding: render-baseline-2cf4be103e26321f-low_contrast
 * Axis: baseline
...
test("Parallax: render-baseline-2cf4be103e26321f-low_contrast", async ({ page }) => {
  const response = await page.goto("/auth?mode=register");
  const isLoginPage = /\/(?:login|sign-in|auth)(?:[/?#]|$)/i.test(new URL(page.url()).pathname);
  const contrastRatio = await page.locator("div.px-6.pb-8:nth-of-type(5) > div.space-y-4 > div:nth-of-type(4) > div.grid.grid-cols-2 > button.py-3.rounded-xl:nth-of-type(1)").evaluate((element) => {
