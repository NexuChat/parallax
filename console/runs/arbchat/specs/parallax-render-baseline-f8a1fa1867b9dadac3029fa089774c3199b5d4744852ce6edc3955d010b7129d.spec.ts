 ...
 * Finding: render-baseline-7f4c5a1cb7cd5198-low_contrast
 * Axis: baseline
...
test("Parallax: render-baseline-7f4c5a1cb7cd5198-low_contrast", async ({ page }) => {
  const response = await page.goto("/about");
  const isLoginPage = /\/(?:login|sign-in|auth)(?:[/?#]|$)/i.test(new URL(page.url()).pathname);
  const contrastRatio = await page.locator("div.min-h-screen > nav.fixed.top-0 > div.relative.max-w-7xl > div.flex.items-center:nth-of-type(1) > a.group.flex").evaluate((element) => {
